...
export class Secp256k1 extends CryptoKey {
    ...
    importFromDid(didKey: string): void {
        ...
        if (!resultKeyType || (resultKeyType !== 'P-256' && resultKeyType !== 'Secp256k1')) {
    ...
    didDocument(method?:SupportedVerificationMethods) {      
        ...
        switch (publicKeyFormat) {
            ...
            case SupportedVerificationMethods.EcdsaSecp256r1VerificationKey2019:
                verificationMethod.publicKeyJwk = createJWK('Secp256k1', this.publicKey(), 'sig')
                break
    ...
    async sign (algorithm:string, data:Uint8Array)
    {
        ...
        const signer = ES256KSigner(this.privateKeyBytes, algorithm === 'ES256K-R')
